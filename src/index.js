...
document.querySelector(".poke-search-form").addEventListener("submit", (e) => {
  ...
  async function poke1True() {
    await fetch(`https://pokeapi.co/api/v2/pokemon/${poke1}/`)
    .then(resp => resp.json())
    .then(data => createCard1(data))
    .catch(sendAlert);
    document.getElementById("alert-text-1").style.display = "none"};

  async function poke2True() {
    wait fetch(`https://pokeapi.co/api/v2/pokemon/${poke2}/`)
    .then(resp => resp.json())
    .then(data => createCard1(data))
    .catch(sendAlert);
  ...
  vsCard.innerHTML = `<p>VS</p>`;
  document.getElementById("pokemon-collection").appendChild(vsCard);
